...
bucket = "{{dag_run.conf['bucket_name']}}"
...
def create_cluster(region_name, cluster_name='Mehul_Cluster' + str(datetime.now()), release_label='emr-6.2.1',master_instance_type='m5.xlarge', num_core_nodes=1, core_node_instance_type='m5.xlarge',**kwargs):
        ...
        BootstrapActions= [
            {
                ...
                'ScriptBootstrapAction': {
                            'Path': 's3://landingzonemehul/Dependency/dependencies.sh',
                        }
            }
			],
...
def read_config(app_config,bucket):
    response = s3.Object(bucket,app_config)
...
def pre_validation(dataset_path,dataset,**kwargs):
    ...
    #working on making it more configurable
...
def create_emr(**kwargs):
    cluster_id = create_cluster(region_name='us-east-1', cluster_name='Mehul_Cluster',  num_core_nodes=1)
    return cluster_id
...
read_config=PythonOperator(
    ...
    op_kwargs={'app_config': app_config, 'bucket':bucket},
    dag=dag)
...
create_cluster = PythonOperator(
    ...
    op_kwargs={'region_name':'us-east-1', 'cluster_name':'Mehul_Cluster', 'num_core_nodes':1},
    dag=dag)
